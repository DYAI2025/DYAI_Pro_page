import { services, insights, projects, projectCategories } from './data.js';
...
const App = {
    ...
    getHeaderHTML() {
        ...
        const navLinks = [
            ...
            { href: 'projects.html', text: 'Projekte' },
    ...
    createProjectCard(project, delay, isFeatured) {
        const statusBadge = {
            'active': '<span class="px-2 py-1 bg-green-500/10 text-green-700 text-xs rounded-full">Active</span>',
            'beta': '<span class="px-2 py-1 bg-blue-500/10 text-blue-700 text-xs rounded-full">Beta</span>',
            'ongoing': '<span class="px-2 py-1 bg-yellow-500/10 text-yellow-700 text-xs rounded-full">Ongoing</span>'
        };

        const metricsKeys = Object.keys(project.metrics);
        const firstThreeMetrics = metricsKeys.slice(0, 3);
        ...
        return `
            <div
                ...
                class="project-card bg-white rounded-xl shadow-subtle overflow-hidden flex flex-col group hover:shadow-strong transition-all duration-300 transform hover:-translate-y-2">
                ...
                <div class="relative h-48 bg-gradient-to-br from-deep-space-blue to-structure-grey overflow-hidden">
                    ...
                    <div class="absolute top-4 left-4">
                        <span class="px-3 py-1 bg-living-coral text-white text-xs font-semibold rounded-full">
                ...
                <div class="p-6 flex-grow flex flex-col">
                    <h3 class="font-sans-custom text-xl font-bold text-deep-space-blue mb-3 group-hover:text-living-coral transition-colors">
                        ${project.title}
                    </h3>
                    <p class="text-sm text-structure-grey mb-4 flex-grow line-clamp-3">
                        ${project.summary}
                    </p>
                    ...
                    <div class="flex flex-wrap gap-2 mb-4">
                        ${project.technologies.slice(0, 3).map(tech => `
                            <span class="px-2 py-1 bg-future-white text-structure-grey text-xs rounded">
                                ${tech}
                            </span>
                        `).join('')}
                    ...
                    <!-- Metrics -->
                    <div class="grid grid-cols-3 gap-4 mb-4 pt-4 border-t border-structure-grey/10">
                        ${firstThreeMetrics.map(key => `
                            <div class="text-center">
                                <div class="text-lg font-bold text-living-coral">
                                    ${project.metrics[key]}
                                </div>
                                <div class="text-xs text-structure-grey/70 capitalize">
                                    ${key.replace('_', ' ')}
                                </div>
                            </div>
                        `).join('')}
                    ...
                    <div class="flex gap-2 mt-auto">
                        ${project.github ? `
                            <a href="${project.github}"
                               ...
                               class="flex-1 flex items-center justify-center gap-2 px-4 py-2 bg-deep-space-blue text-white rounded-lg hover:bg-opacity-90 transition-all text-sm font-medium">
                        ...
                        ${project.demo ? `
                            <a href="${project.demo}"
                               class="flex-1 flex items-center justify-center gap-2 px-4 py-2 bg-living-coral text-white rounded-lg hover:bg-opacity-90 transition-all text-sm font-medium">
                                <i data-lucide="play-circle" class="w-4 h-4"></i>
                                <span>Demo</span>
                            </a>
                        ` : ''}
                        ${!project.demo && !project.github ? `
                            <div class="flex-1 flex items-center justify-center px-4 py-2 bg-structure-grey/10 text-structure-grey rounded-lg text-sm">
                                Details folgen
                            </div>
                        ` : ''}
                    </div>
