...
const App = {
  ...
  getHeaderHTML() {
    ...
    const navLinks = [
      ...
      { href: "instafile.html", text: "InstaFile" },
    ];

    const moreLinks = [
      { href: "ueber.html", text: "Über" },
      { href: "contact.html", text: "Kontakt" },
      { href: "impressum.html", text: "Impressum" },
      { href: "privacy.html", text: "Datenschutz" },
    ];
    ...
    return `
            <nav class="bg-white/80 backdrop-blur-lg shadow-subtle">
                <div class="container mx-auto px-4">
                        ...
                        <div class="hidden md:flex items-center space-x-2">
                            ${navLinks
                              .map(
                                ...
                                <a href="${
                                  link.href
                                }" class="nav-link font-sans-custom font-medium px-4 py-2 rounded-md transition-colors duration-300 ${
                                  currentPage === link.href
                                    ? "text-living-coral"
                                    : "text-structure-grey hover:text-deep-space-blue"
                                }">
                                    ${link.text}
                                </a>
                            `
                            ...
                            <div class="relative group">
                                <button type="button" class="nav-link font-sans-custom font-medium px-4 py-2 rounded-md transition-colors duration-300 flex items-center gap-1 text-structure-grey hover:text-deep-space-blue">
                                    Mehr
                                    <i data-lucide="chevron-down" class="w-4 h-4"></i>
                                </button>
                                <div class="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg border border-structure-grey/10 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
                                    ${moreLinks
                                      .map(
                                        (link) => `
                                        <a href="${link.href}" class="block px-4 py-2 text-sm text-structure-grey hover:bg-future-white transition-colors">
                                            ${link.text}
                                        </a>
                                    `
                                      )
                    ...
                    <!-- Mobile Menu -->
                    <div id="mobile-menu" class="hidden md:hidden bg-white border-t border-structure-grey/10 py-4">
                        <div class="space-y-2">
                            ${navLinks
                              .map(
                                (link) => `
  ...
  getFooterHTML() {
    return `
            <div class="bg-deep-space-blue text-future-white/80">
                <div class="container mx-auto px-4 py-12">
                    <div class="grid gap-10 md:grid-cols-4">
                        ...
                        <div>
                            <h3 class="font-sans-custom text-lg font-semibold text-future-white mb-3">Navigation</h3>
                            <ul class="space-y-2 text-sm">
                                <li><a href="philosophy.html" class="hover:text-living-coral transition-colors">Philosophie</a></li>
                                <li><a href="services.html" class="hover:text-living-coral transition-colors">Angebote</a></li>
                                <li><a href="insights.html" class="hover:text-living-coral transition-colors">Einblicke</a></li>
                                <li><a href="instafile.html" class="hover:text-living-coral transition-colors">InstaFile</a></li>
                            </ul>
                        </div>
                        <div>
            ...
            <div>
              <h3 class="font-sans-custom text-lg font-semibold text-future-white mb-3">Kontakt & Info</h3>
              <ul class="space-y-2 text-sm">
                <li><a href="contact.html" class="hover:text-living-coral transition-colors">Kontaktformular</a></li>
                <li><a href="impressum.html" class="hover:text-living-coral transition-colors">Impressum (Angaben)</a></li>
                <li><a href="privacy.html" class="hover:text-living-coral transition-colors">Datenschutz</a></li>
              </ul>
              <p class="text-xs mt-4 text-future-white/60">Persönliche Angaben nur im Impressum – Fokus auf Inhalte.</p>
            </div>
                    </div>
  ...
  initPageSpecifics() {
    ...
    document
      .querySelectorAll("[data-initial-hidden]")
  ...
  loadServices() {
    ...
    services.forEach((service, index) => {
      html += `
                <div data-scroll-fade style="animation-delay: ${
                  index * 150
                }ms;" class="bg-white rounded-xl shadow-subtle p-8 flex flex-col hover:shadow-strong transition-shadow duration-300">
                    <div class="flex items-center justify-center w-16 h-16 rounded-full bg-living-coral/10 mb-6">
                        <i data-lucide="${
                          service.icon
                        }" class="w-8 h-8 text-living-coral"></i>
                    </div>
                    <h3 class="font-sans-custom text-2xl font-bold text-deep-space-blue mb-4">${
                      service.title
                    }</h3>
                    <p class="text-base text-structure-grey flex-grow">${
                      service.description
                    }</p>
  ...
  loadInsights() {
    ...
    insights.forEach((post, index) => {
      html += `
                <div data-scroll-fade style="animation-delay: ${
                  index * 150
                }ms;" class="bg-white rounded-xl shadow-subtle overflow-hidden flex flex-col group hover:shadow-strong transition-shadow duration-300">
                    <div class="p-8 flex-grow flex flex-col">
                        <p class="font-sans-custom text-sm text-structure-grey/70 mb-2">${
                          post.category
                        }</p>
                        <h3 class="font-sans-custom text-xl font-bold text-deep-space-blue mb-3 flex-grow">${
                          post.title
                        }</h3>
                        <p class="text-base text-structure-grey mb-4">${
                          post.summary
                        }</p>
                        <div class="mt-auto">
  ...
  handleContactForm() {
    ...
    form.addEventListener("submit", (e) => {
      ...
      feedbackEl.textContent =
        "Vielen Dank! Ihre Nachricht wird verarbeitet...";
  ...
  initVoiceAssistant() {
    ...
    voiceButton.className =
      "fixed bottom-6 right-6 bg-living-coral text-white p-4 rounded-full shadow-lg hover:scale-110 transition-transform duration-300 z-50";
  ...
  processVoiceCommand(command) {
    ...
    this.showNotification(
      "Befehl nicht erkannt. Versuchen Sie: Philosophie, Angebote, Kontakt"
    );
  },
  ...
  initChatBot() {
    ...
    chatButton.className =
      "fixed bottom-6 left-6 bg-deep-space-blue text-white p-4 rounded-full shadow-lg hover:scale-110 transition-transform duration-300 z-50";
    ...
    chatInterface.className =
      "fixed bottom-24 left-6 w-80 h-96 bg-white rounded-lg shadow-2xl hidden flex-col z-50";
  ...
  generateAIResponse(message) {
    const responses = {
      philosophie:
        "Unsere Philosophie basiert auf vier Kernprinzipien: Partnerschaft statt Werkzeug, proaktive Zukunftsgestaltung, menschzentrierte Evolution und bewusste Auseinandersetzung.",
      angebote:
        "Wir bieten Beratung, Workshops und kuratierte Lernreisen rund um den bewussten Einsatz von KI an.",
      kontakt:
        "Nutzen Sie gerne unser Kontaktformular oder sehen Sie rechtliche Angaben im Impressum. Wir melden uns zeitnah zurück.",
      ki: "Wir verstehen AI als Augmented Intelligence - eine Erweiterung des Menschen und seiner Fähigkeiten.",
      training:
        "Unsere Trainings sind praxisorientiert und auf Ihre spezifischen Bedürfnisse zugeschnitten.",
      zukunft:
        "In der Zukunft wird KI nahtlos in unsere Gesellschaft integriert sein - als Partner, nicht als Ersatz.",
    };
  ...
  initSmartNavigation() {
    ...
    window.addEventListener("scroll", () => {
      ...
      if (
        scrollDepth > 80 &&
        !document.querySelector(".smart-nav-suggestion")
      ) {
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            ...
            const timeSpent =
              Date.now() - parseInt(entry.target.dataset.viewTime);
  ...
  showSmartSuggestion() {
    ...
    suggestionEl.className =
      "smart-nav-suggestion fixed bottom-20 right-6 bg-white p-4 rounded-lg shadow-lg border-l-4 border-living-coral z-50 transform translate-x-full";
  ...
  showNotification(message) {
    ...
    notification.className =
      "fixed top-4 right-4 bg-living-coral text-white px-6 py-3 rounded-lg shadow-lg z-50 transform translate-x-full";
